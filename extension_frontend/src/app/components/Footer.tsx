const socialLinks = [
  {
    name: 'Twitter / X',
    href: 'https://twitter.com/CSCegov',
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
        <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
      </svg>
    ),
  },
  {
    name: 'Facebook',
    href: 'https://facebook.com/CSCeGov',
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
        <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
      </svg>
    ),
  },
  {
    name: 'YouTube',
    href: 'https://youtube.com/@CSCegov',
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
        <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
      </svg>
    ),
  },
  {
    name: 'Instagram',
    href: 'https://instagram.com/CSCegov',
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881z" />
      </svg>
    ),
  },
  {
    name: 'LinkedIn',
    href: 'https://linkedin.com/company/csc-egov',
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
        <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
      </svg>
    ),
  },
];

export default function Footer() {
  return (
    <footer className="w-full mt-auto">
      {/* Tricolor strip */}
      <div className="flex h-1">
        <div className="flex-1 bg-[#FF9933]" />
        <div className="flex-1 bg-white" />
        <div className="flex-1 bg-[#138808]" />
      </div>

      {/* Main footer */}
      <div className="bg-navy text-white">
        <div className="max-w-7xl mx-auto px-4 py-10">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-8">
            {/* Column 1: About */}
            <div>
              <h3 className="text-saffron font-bold text-lg mb-3">CSC सहायक</h3>
              <p className="text-slate-300 text-sm leading-relaxed">
                छत्तीसगढ़ के ग्रामीण क्षेत्रों में CSC ऑपरेटरों के लिए AI-संचालित नागरिक सेवा प्रबंधन प्रणाली।
              </p>
              <p className="text-slate-400 text-xs mt-2">
                AI-powered citizen service management for CSC operators in rural Chhattisgarh.
              </p>
            </div>

            {/* Column 2: Quick Links */}
            <div>
              <h4 className="text-saffron font-semibold text-sm mb-3 pb-2 border-b border-white/10">
                त्वरित लिंक | Quick Links
              </h4>
              <ul className="space-y-2">
                {[
                  { hi: 'मुख्य पृष्ठ', en: 'Home' },
                  { hi: 'नया आवेदन', en: 'New Application' },
                  { hi: 'आवेदन इतिहास', en: 'Application History' },
                  { hi: 'सहायता केंद्र', en: 'Help Center' },
                  { hi: 'प्रतिक्रिया', en: 'Feedback' },
                ].map((link) => (
                  <li key={link.en}>
                    <a
                      href="#"
                      className="text-slate-300 hover:text-saffron transition-colors text-sm"
                    >
                      {link.hi} / {link.en}
                    </a>
                  </li>
                ))}
              </ul>
            </div>

            {/* Column 3: Contact */}
            <div>
              <h4 className="text-saffron font-semibold text-sm mb-3 pb-2 border-b border-white/10">
                संपर्क | Contact
              </h4>
              <div className="space-y-2 text-sm text-slate-300">
                <p className="text-white font-medium">CHIPS Chhattisgarh</p>
                <p>छत्तीसगढ़ इन्फोटेक प्रमोशन सोसाइटी</p>
                <div className="flex items-start gap-2">
                  <span className="text-saffron mt-0.5">📍</span>
                  <span>Naya Raipur, Chhattisgarh</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-saffron">📞</span>
                  <span>0771-4013851</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-saffron">✉️</span>
                  <span>chipscg@cgstate.gov.in</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-saffron">🌐</span>
                  <span>www.cgstate.gov.in</span>
                </div>
              </div>
            </div>

            {/* Column 4: Social Media */}
            <div>
              <h4 className="text-saffron font-semibold text-sm mb-3 pb-2 border-b border-white/10">
                सोशल मीडिया | Social Media
              </h4>
              <p className="text-slate-400 text-xs mb-4">
                नवीनतम अपडेट के लिए हमें फॉलो करें | Follow us for latest updates
              </p>
              <div className="flex flex-wrap gap-3">
                {socialLinks.map((link) => (
                  <a
                    key={link.name}
                    href={link.href}
                    target="_blank"
                    rel="noopener noreferrer"
                    title={link.name}
                    className="w-10 h-10 rounded-full bg-white/10 hover:bg-saffron flex items-center justify-center transition-all duration-200 hover:scale-110"
                  >
                    {link.icon}
                  </a>
                ))}
              </div>
            </div>
          </div>

          {/* Bottom bar */}
          <div className="border-t border-white/10 pt-4 flex flex-col sm:flex-row items-center justify-between gap-3">
            <p className="text-slate-400 text-xs text-center sm:text-left">
              © 2026 CHIPS Chhattisgarh | छत्तीसगढ़ इन्फोटेक प्रमोशन सोसाइटी | सर्वाधिकार सुरक्षित
            </p>
            <div className="flex items-center gap-3 text-xs">
              <a href="#" className="text-slate-400 hover:text-white transition-colors">गोपनीयता नीति</a>
              <span className="text-slate-600">|</span>
              <a href="#" className="text-slate-400 hover:text-white transition-colors">उपयोग की शर्तें</a>
              <span className="text-slate-600">|</span>
              <a href="#" className="text-slate-400 hover:text-white transition-colors">अस्वीकरण</a>
              <span className="text-slate-500 text-[10px]">Last Updated: June 2026</span>
            </div>
          </div>
        </div>
      </div>
    </footer>
  );
}
